...
#### 6.1 Agentes Reactivos Simples (`AgenteGuia`)
Su arquitectura no contempla memoria histórica. Opera puramente mediante un conjunto de reglas Condición-Acción evaluando los cobros de boletos en la puerta de los museos.

```python
# Código íntegro de AgenteGuia en agentes_ia.py
class AgenteGuia:
...
#### 6.2 Agentes Reactivos Basados en Modelos (`AnimadorMovimiento`)
Contiene una representación interna del mundo físico. Ingiere una polilínea GPS cruda, y dibuja el marcador sobre las calles en la pantalla ajustando los cuadros por segundo según un acelerador simulado.

```python
# Código íntegro de AnimadorMovimiento en agentes_ia.py
...
#### 6.3 Agentes Basados en Objetivos (`AgenteTransporte`)
Posee la meta general. Delega instrucciones seriales y ordenadas al agente físico de movimiento, cerciorándose de que cada museo y parada se concluya en orden.

```python
# Código íntegro de AgenteTransporte en agentes_ia.py
...
#### 6.4 Agentes Basados en Utilidad (`AgenteBuscador`)
La clase máxima de inteligencia. Mide matemáticamente la satisfacción (o "Utilidad") de cada ruta DFS posible, ponderando el balance óptimo.

```python
# Código de la estructura principal de AgenteBuscador en agentes_ia.py 
# (La inteligencia algorítmica profunda se desglosa matemáticamente en los Puntos 13, 14, 15 y 16)
class AgenteBuscador(QThread):
    ...
    def run(self):
        try:
            ...
            # ---> INVOCACIÓN A DFS, PODA Y MACROOPERADORES (Desglosado en Puntos 13, 14, 15 y 16)
...
### 18. Representación Topológica y Visualización de Grafos (grafo_visual.py)
A medida que la Inteligencia Artificial resuelve el problema de enrutamiento y genera una solución, es vital poder auditar y analizar las decisiones que tomó el algoritmo. Para esto, se implementó un sistema de visualización de grafos estáticos de manera controlada y bajo demanda.

**Fundamento Teórico:**
La ruta encontrada por la IA es esencialmente un grafo dirigido $G = (V, E)$, donde los Vértices ($V$) son los museos o paradas de transporte, y las Aristas dirigidas ($E$) son los trayectos físicos. En lugar de forzar a la computadora a dibujar este grafo durante el proceso de búsqueda intensiva (lo que saturaría la RAM), se diseñó un módulo abstracto y desacoplado (`grafo_visual.py`) que solo construye y dibuja la estructura geométrica cuando el usuario lo solicita explícitamente sobre una ruta ya finalizada.

**Explicación del Código:**
El módulo combina el poder algorítmico de la librería matemática `NetworkX` para la estructuración lógica de los datos, y `Matplotlib` para el renderizado vectorial, encapsulando todo como un lienzo dentro de una ventana independiente nativa de `PyQt5`.

```python
# Módulo de visualización topológica (grafo_visual.py)
class VentanaGrafo(QMainWindow):
    def __init__(self, ruta_optima):
        # ... inicialización de ventana ...
        self.detalles = ruta_optima.get("geometrias", [])
        self.grafo = nx.DiGraph() # Inicialización de un Grafo Dirigido (Aristas con flechas)
        self.construir_grafo()
        self.dibujar()

    def construir_grafo(self):
        # Itera sobre cada segmento logístico de la solución encontrada por la IA
        for tramo in self.detalles:
            origen = str(tramo.get("origen"))
            destino = str(tramo.get("destino"))
            
            # Extrae la heurística de costo para etiquetar la arista
            modo = tramo.get("modo", "?")
            distancia = tramo.get("distancia", 0)
            tiempo = tramo.get("tiempo", 0)
            costo = tramo.get("costo", 0)
            
            # Etiqueta visual informativa de la Arista
            label = f"{modo}\n{distancia:.2f} km\n{tiempo:.1f} min\nBs {costo:.2f}"
            self.grafo.add_edge(origen, destino, label=label)

    def dibujar(self):
        # Renderizado usando un algoritmo de simulación de resortes espaciales (spring_layout)
        pos = nx.spring_layout(self.grafo)
        
        # Inyección de las capas de gráficos al lienzo de Matplotlib
        ...
        # Plasmado de las etiquetas de costo sobre las aristas
...
```
El motor de dibujo utiliza la función algorítmica `nx.spring_layout()`, que implementa un modelo de posicionamiento de "fuerza dirigida". Básicamente, simula fuerzas de repulsión (como imanes) entre todos los nodos mientras las aristas actúan como resortes que tiran de ellos. Como resultado, el grafo se auto-organiza dinámicamente en la pantalla buscando el equilibrio espacial, evitando en lo posible que las líneas se crucen y garantizando su fácil interpretación y legibilidad humana.

### 19. Manual Operativo Completo para el usuario del simulador
...
7. **Análisis Topológico (Opcional):** Antes de presionar "Iniciar", o si simplemente desea estudiar la estructura matemática del recorrido de forma estática, seleccione una ruta de la lista y haga clic en el nuevo botón **"Ver Grafo"** (color morado). Se abrirá una ventana emergente generada por `grafo_visual.py` que modela todo el viaje con nodos y aristas interactivas, permitiéndole entender cada etapa logística que conforma la ruta seleccionada.
